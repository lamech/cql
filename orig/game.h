...
public:
    moveT *     CurrentMove; //made public by stiller
    void     SaveState (); //made public by stiller
    errorT   RestoreState();// made public by stiller

    moveT * my_getcurrentmove();
    errorT WritePGN_changed(TextBuffer * tb, uint stoplocation);
    errorT WriteToPGN_changed(TextBuffer * tb);
    void handleinitialcomment_changed(TextBuffer * tb);
    Game()      { Init(); }
    ~Game() {
