...
TextBuffer::SetBufferSize (uint length)
{
    ...
    Buffer = new char[length+128]; //+128 added by stiller 2/26/2019 to workaround buffer overflow in Newline()
    BufferSize = length;
    ByteCount = Column = LineCount = 0; LineIsEmpty = 1;
    Current = Buffer;
    for(int i=0;i<length+128;++i) // added by stiller 2/26/2019 for reproducibility of bugs
      Buffer[i]=0;
    *Current = 0;
}
