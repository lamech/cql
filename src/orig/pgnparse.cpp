...
/*method GetRestOfWord_AlphaNum() was copied from GetRestOfWord_letters
  and is a copy of GetRestOfWord_AlphaNum except that isalnum instead of
  isAlpha is used. By analogy with the test for "null" as a null move.
  Added by stiller 12/26/2020 build 9.245 to parse Z0
*/
void
PgnParser::GetRestOfWord_AlphaNum (char * buffer)
{
    int ch = GetChar();
    while (isalnum(ch)) {
        *buffer++ = ch;
...
PgnParser::GetGameToken (char * buffer, uint bufSize)
{
    ...
    // Following section added by Stiller 12/26/2020 for Z0 checking per Yves bug report
    if (ch == 'Z') {
        GetRestOfWord_AlphaNum (buf);
        if (strEqual (buffer, "Z0")) {
            return TOKEN_Move_Null;
        }
        return TOKEN_Invalid;
    }
...
errorT
PgnParser::ParseMoves (Game * game, char * buffer, int bufSize)
{
    errorT err = OK;
