...
StrTree<C>::MakeSubTree (int size, uint depth)
{
    ...
    int nLeft, nRight;//, mid the "mid" variable removed 9/14/2022 by stiller for warnings
    nLeft = (size - 1) / 2;
    nRight = size - nLeft - 1;
    //    mid = nLeft + 1; commented out 9/14/2022 by stiller to prevent compiler warning
