...
//
errorT
Game::WritePGN_changed (TextBuffer * tb, uint stopLocation)
{
    char temp [255];
    char dateStr [20];
    const char * newline = "\n";
    ...
    handleinitialcomment_changed(tb);
...
void Game::handleinitialcomment_changed(TextBuffer * tb){
  if(FirstMove->comment==NULL) return;
  tb->PrintString("{");
  //  tb->PrintString(" "); //commented out for 9.325 as unnecessary
  tb->PrintString(FirstMove->comment);
  tb->PrintLine("}");
}

errorT
Game::WriteToPGN_changed(TextBuffer * tb){
  return WritePGN_changed(tb,0);
}
  bool
  Position::myIsKingInMate ()
{
...
bool Position::myIsKingInCheck(){
  return CalcNumChecks (GetKingSquare(ToMove),CheckSquares)>0;
}
bool Position::myIsKingInStalemate(){
  GenerateMoves();
...
/*This is necessary because some PGN files (likely from ChessBase), including heijden.pgn, 
  begin with non-ASCII characters, having to do with obscure Unicode semantics. So this
  just skips them all. A separate requirement is for the PgnSource file reads. Without this,
  the first game of the read pgn file by SCID has a garbled first comment.
*/
void MFile::skipNonAscii(){
  uassert(Handle&&Location==0,"mf skipnonascii");
  int c;
  while(true){
    c=fgetc(Handle);
    eassert(c>0,"Unexpected end of file in skipNonAscii: possibly not valid PGN file");
    if(c>127)
      Location++;
    else{
      int u=ungetc(c,Handle);
      eassert(u>=0,"ungetc failed skipNonAscii: file problem reading PGN file");
      if(0&&Location)
